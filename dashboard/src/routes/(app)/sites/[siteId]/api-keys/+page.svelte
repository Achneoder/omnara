<script lang="ts">
  ...
  async function load() {
    ...
    error = '';
    try {
      keys = await api.apiKeys.list(siteId);
    } catch (err) {
  ...
  async function handleCreate(e: SubmitEvent) {
    ...
    createError = '';
    try {
      const result = await api.apiKeys.create(siteId, { label: newLabel });
      createdKey = result;
      keys = [...keys, result];
  ...
  async function handleRevoke(key: ApiKey) {
    ...
    try {
      await api.apiKeys.revoke(siteId, key.id);
...
<svelte:head>
  <title>API Keys — {data.site.name} — omnara</title>
</svelte:head>
...
<div class="p-8">
  <div class="mb-6 flex items-center justify-between">
    <div>
      ...
      <p class="mt-1 text-sm text-gray-500">
        Manage API keys for agent access to {data.site.name}.
      </p>
    </div>
  ...
  {:else if keys.length === 0}
    <EmptyState
      title="No API keys"
      description="Generate an API key for agents to authenticate with this site."
    >
      {#snippet action()}
